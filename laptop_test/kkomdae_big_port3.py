...
class TestApp(ttkb.Window):
    ...
    def _init_variables(self) -> None:
        ...
        # 카드 스타일 추가
        self._style.configure("Card.TFrame", background="white", borderwidth=2, relief="raised")
        self._style.configure("CardContent.TFrame", background="#f8f9fa")  # 연한 회색 배경
    ...
    def create_test_items(self) -> None:
        ...
        2행 3열의 격자 배치로 구성하고 카드 형태로 표시합니다.
        """
        ...
        # 2행으로 균등하게 분배 (각 행의 최소 높이 250으로 증가)
        for row in range(2):
            test_frame.grid_rowconfigure(row, weight=1, minsize=250)
        # 3열로 균등하게 분배 (각 열의 최소 폭 280으로 증가)
        for col in range(3):
            test_frame.grid_columnconfigure(col, weight=1, minsize=280)
    ...
    def create_test_item(self, parent, name: str, row: int, col: int) -> None:
        """
        각 테스트 항목의 UI를 카드 형태로 생성하고, 격자에 배치합니다.
        """
        # 카드형 컨테이너 프레임 생성 (그림자 효과를 위한 외부 프레임)
        card_frame = ttkb.Frame(parent, padding=5)
        card_frame.grid(row=row, column=col, padx=15, pady=15, sticky="nsew")
        
        # 실제 내용을 담을 내부 프레임 (테두리와 배경색으로 카드 느낌 부여)
        frame = ttkb.Frame(card_frame, padding=10, width=250, height=200, 
                        bootstyle="light", # 밝은 배경색
                        borderwidth=1, relief="solid") # 테두리 추가
        frame.pack(fill="both", expand=True)
        frame.pack_propagate(False)  # 프레임 크기 고정
        
        # [Row 0] 아이콘 전용 프레임 (고정 크기, 최상단에 배치)
        self.icon_frame = ttkb.Frame(frame, width=350, height=150, style="IconFrame.TFrame")
        self.icon_frame.pack(pady=(0, 10), padx=10, fill="x")
        self.icon_frame.pack_propagate(False)  # 프레임 크기 고정
        
        # 아이콘 프레임 저장
        if not hasattr(self, 'icons'):
            self.icons = {}
        self.icons[name] = self.icon_frame
        ...
        # 아이콘 이미지 대신 텍스트 타이틀 표시
        self.icon_label = ttkb.Label(
            self.icon_frame, 
            text=name,
            font=("Arial", 90, "bold"), 
            foreground="white",
            background=self.test_status_colors["테스트 전"],
            justify='center'
        )
        ...
        # desc_label.pack(pady=(5, 0), fill="x")
        ...
        status_label.pack(pady=(5, 0), fill="x")
        self.test_status_labels[name] = status_label

        # 각 테스트 유형에 따른 추가 버튼 처리
        if name == "키보드":
            ...
            self.failed_keys_button.pack(pady=(5, 0), fill="x")
            ...
            # USB 포트 상태 레이블들을 담을 프레임 (가로 배치)
            usb_ports_frame = ttkb.Frame(frame)
            usb_ports_frame.pack(pady=(5, 0), fill="x")
            
            self.usb_port = []
            for port in range(1, 4):
            # 각 포트 레이블을 가로로 배치 (side='left')
                port_label = ttkb.Label(
                    usb_ports_frame,
                    image=self.status_images["USB테스트 전"][port],
                )
                port_label.pack(side='left', padx=5, expand=True, fill='x')
            ...
            self.usb_refresh_button.pack(pady=(5, 0), fill="x")
            ...
            self.battery_report_button.pack(pady=(5, 0), fill="x")
